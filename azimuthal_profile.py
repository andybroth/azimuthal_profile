...
from /home/andyr/src/frb/radial_profile1 import log, get_amiga_data, read_amiga_center, \
read_amiga_rvir, smooth_amiga, make_off_axis_projection

def find_angular_momentum(sp, c):
    """
    Finds the average angular momentum of a sphere of particles around a center point c
    """
    pt = 'PartType0'
...
if __name__ == '__main__':
    ...
    for fn in yt.parallel_objects(fns):
       ...
       # Figure out centroid and r_vir info
		...
		phi = np.abs(np.pi - np.arctan(np.abs(py / px)))
        ...
        for i, ion_field in enumerate(ion_fields):
            ...
            if dset not in cdens_file.keys():
                ...
                cdens_file.flush()
        frb = make_off_axis_projection(ds, b1, ang_mom, full_other_fields, \
